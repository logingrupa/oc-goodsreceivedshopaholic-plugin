...
- [~] **APPLY-08**: Override-reimport (D12): when operator ticks "Override and re-import" on a duplicate invoice, ApplyOrchestrator re-applies the new lines additively on top of the prior apply. New `Invoice` row with `override_of_invoice_id` = prior. UX warning displayed before submit; operator confirms by typed string. *(parse-side closed 2026-04-29 in plan 03-06 — `ParseAndPersistOrchestrator::runOverride(string $sHtmlContent, string $sSourceFilename, int $iPriorInvoiceId, int $iAppliedByUserId): Invoice` explicit method (NOT a flag) per D-26; new Invoice gets `override_of_invoice_id=$iPriorInvoiceId` + `invoice_number='<orig>-OVR-<priorId>'` suffix to satisfy UNIQUE index while the prior invoice keeps its canonical number. Duplicate-check is SKIPPED in override mode (the prior invoice IS the explicit reference). Pinned by `runOverride creates a NEW invoice with override_of_invoice_id pointer, NO duplicate-exception` Pest case. Apply-side (ApplyOrchestrator runs UNCHANGED on override invoices — additive D-12 add-on-top semantics emerge naturally) ships in plan 03-07. UX confirmation flow ships in Phase 4 UI-09/10.)*
...
| APPLY-08 | Phase 3 | Parse-side closed (plan 03-06, 2026-04-29); apply-side pending (plan 03-07) |
