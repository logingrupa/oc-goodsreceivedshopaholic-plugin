...
phpmd:
	cd $(PROJECT_ROOT) && $(VENDOR_BIN)/phpmd $(PLUGIN_DIR)/classes,$(PLUGIN_DIR)/console,$(PLUGIN_DIR)/controllers,$(PLUGIN_DIR)/models,$(PLUGIN_DIR)/Plugin.php text $(PLUGIN_DIR)/phpmd.xml
...
lint-settings-accessor:
	...
	@if grep -rn 'Settings::get(' $(PLUGIN_DIR)/classes $(PLUGIN_DIR)/console $(PLUGIN_DIR)/controllers $(PLUGIN_DIR)/models $(PLUGIN_DIR)/Plugin.php 2>/dev/null | grep -v 'classes/support/SettingsAccessor.php' | grep -q .; then \
		echo "QA-09 VIOLATION: Settings::get( found outside SettingsAccessor.php"; \
		grep -rn 'Settings::get(' $(PLUGIN_DIR)/classes $(PLUGIN_DIR)/console $(PLUGIN_DIR)/controllers $(PLUGIN_DIR)/models $(PLUGIN_DIR)/Plugin.php 2>/dev/null | grep -v 'classes/support/SettingsAccessor.php'; \
		exit 1; \
	fi
